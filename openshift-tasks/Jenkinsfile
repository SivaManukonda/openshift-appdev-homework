  ...
  node('skopeo-pod') {
    ...
    dir('openshift-tasks') {
      ...
      stage('Deploy to Dev') {
        ...
        sh("oc delete configmap tasks-config --ignore-not-found=true -n ${devProject}") 
        script {
      ...
      stage('Blue/Green Production Deployment') {
        ...
        // sh("oc delete configmap ${destApp}-config --ignore-not-found=true -n ${prodProject}") 
        script {
          openshift.withCluster() {
      ...
      stage('Auto switch over to new Version') {
